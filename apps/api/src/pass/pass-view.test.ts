...
describe(passView, () => {
  it('maps an active ADMIT to VALID with tier label and short holder', () => {
    const view = passView(
      { branding: null, holder: HOLDER, level: 'bearer', tier: 2, uid: UID },
  ...
  it('surfaces the REJECT reason as the status', () => {
    const view = passView(
      { branding: null, holder: HOLDER, level: 'bearer', tier: 0, uid: UID },
  ...
  it('reports UNKNOWN when the chain could not be read and — for a missing holder', () => {
    const view = passView({ branding: null, holder: null, level: 'private', tier: 1, uid: UID }, null)
    expect(view).toMatchObject({ holderShort: '—', status: 'UNKNOWN' })
  })

  it('falls back to a numbered label for a tier beyond the known ones', () => {
    const view = passView({ branding: null, holder: HOLDER, level: 'bearer', tier: 9, uid: UID }, null)
