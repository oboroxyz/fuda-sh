...
`env.local.vars` block already carries the matching (deterministic) values, and
`pnpm --filter api dev` runs `wrangler dev --env local`, so the quick start above
works as written. Both the top-level and the `env.local` D1 block name the same
database, so `migrate:local` targets the same local database that `--env local`
then serves; `--env local` reads `.dev.vars.local` when present and falls back to
