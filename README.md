...
Run `pnpm dev` for the complete local stack on the fixed ports shown in the
table, or start one surface with `pnpm dev:api`, `pnpm dev:app`,
`pnpm dev:gate`, or `pnpm dev:dash`. A selective frontend command needs the
matching `pnpm dev:api` running in another terminal for live api calls.
