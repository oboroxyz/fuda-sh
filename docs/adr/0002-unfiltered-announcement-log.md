...
- **Status**: Superseded by ADR 0003 on 2026-09-06. Preserved as the historical API-cache decision.
...
- A caller allow-list on the Announcer's `caller` field would break the moment an issuer other than fuda's signer announces (the issuer-signed issuance path); the receiver's view tag is the filter ERC-5564 intends.
