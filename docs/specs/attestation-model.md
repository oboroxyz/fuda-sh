...
token is stored — `address`, `audience` (`operator` or `member`, existing rows default
to `operator`), nullable `issuer_id`, `created_at`, `expires_at` = created
...
`challenges` table under an `operator:<address>` or `member:<address>` subject,
with the gate's TTL and sweep. Member sessions never have an issuer binding. An issuer is a product and branding entity only: every attestation
