...
The endpoint is public and does not use `ADMIN_TOKEN`. It and
`/announcements` share one fixed hourly D1 counter: together they admit 120
requests per client IP and require `CF-Connecting-IP`. The resolver allowlist
prevents the service from signing responses for arbitrary verifier contracts.
